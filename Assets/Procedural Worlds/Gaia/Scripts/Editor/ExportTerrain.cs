...
namespace Gaia
{
    ...
    class ExportTerrain : EditorWindow, IPWEditor
    {
        ...
        private void ExportTerrainPanel(bool helpEnabeld)
        {
            ...
            {
                m_exportPath = GaiaDirectories.GetExportDirectory() + GaiaDirectories.TERRAIN_MESH_EXPORT_DIRECTORY;
            }
            m_exportPath = m_editorUtils.TextField("ExportDirectory", m_exportPath);

            if (m_editorUtils.Button("ExportDirectoryOpen", GUILayout.Width(80)))
            {
                string path = EditorUtility.SaveFolderPanel(m_editorUtils.GetTextValue("ExportDirectoryWindowTitle"), m_exportPath, "");
                if (path.Contains(Application.dataPath))
                {
                    m_exportPath = GaiaDirectories.GetPathStartingAtAssetsFolder(path);
                }
                else
                {
                    m_exportPath = path;
            ...
            if (!m_exportPath.StartsWith("Assets"))
            {
                m_workingExportPath = GaiaDirectories.GetTempExportPath();
                m_copyToPath = m_exportPath;
            ...
            {
                m_workingExportPath = m_exportPath;
                m_copyToPath = "";
            }

            m_editorUtils.InlineHelp("ExportDirectory", helpEnabeld);
            ...
            if (m_editorUtils.Button("ExportButton"))
            {
                ...
                if (m_copyToPath != "")
                {
                    ...
                    Debug.Log("Copied " + m_createdFiles.Count.ToString() + " files to the final export directory " + m_exportPath);
                }
            }
        ...
        void Export(ExportTerrainLODSettings LODSettings)
        {
            ...
            foreach (Terrain terrain in selectedTerrains)
            {
                ...
                EditorUtility.DisplayProgressBar("Exporting Mesh " + m_currentTerrainCount.ToString() + " of " + maxTerrainCount, "Mesh Export (Can take some time, please be patient...)", (float)m_currentTerrainCount / (float) maxTerrainCount);
                ...
                var modelImporter = ModelImporter.GetAtPath(objFileName) as ModelImporter;
                if (modelImporter != null)
                {
                    modelImporter.materialImportMode = ModelImporterMaterialImportMode.None;
                    AssetDatabase.ImportAsset(objFileName);
                }
                ...
                if (LODSettings.m_exportTextures)
                {
                    ...
                    switch (LODSettings.m_textureExportMethod)
                    {
                        ...
                        case TextureExportMethod.OrthographicBake:
                            textureFileName = m_workingExportPath + Path.DirectorySeparatorChar + LODSettings.namePrefix + terrain.name + "_Baked";
                ...
                {
                    normalMapFileName = m_workingExportPath + Path.DirectorySeparatorChar + LODSettings.namePrefix  + terrain.name + "_Normal.png";
        ...
        {
            string fname = m_workingExportPath + Path.DirectorySeparatorChar + LODPrefix + terrain.name + "_BaseMap";
