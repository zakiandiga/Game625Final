...
namespace Gaia
{
    ...
    public class SpawnerEditor : PWEditor, IPWEditor
    {
        ...
        private ResourceProtoProbe m_probePrototypeBeingDrawn;
        ...
        private void DrawSpawnRuleMaskListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            if (m_maskListBeingDrawn != null && m_maskListBeingDrawn.Length > index)
            {
                ...
                MaskListButtonCommand mlbc = ImageMaskListEditor.DrawMaskListElement(rect, index, m_maskListBeingDrawn, ref m_collisionMaskListBeingDrawn, m_editorUtils, Terrain.activeTerrain, false, copiedImageMask, m_spawnRulesHeader.normal.background, m_errorHeader.normal.background, m_gaiaSettings, SessionManager);
        ...
        private void DrawSpawnerMaskListElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            MaskListButtonCommand mlbc = ImageMaskListEditor.DrawMaskListElement(rect, index, m_spawner.m_settings.m_imageMasks, ref m_collisionMaskListBeingDrawn, m_editorUtils, Terrain.activeTerrain, false, copiedImageMask, m_spawnRulesHeader.normal.background, m_errorHeader.normal.background, m_gaiaSettings, SessionManager);
            switch (mlbc)
            {
                case MaskListButtonCommand.Delete:
                    ...
                    switch (m_resourceImportType)
                    {
                        ...
                        case ImportableResourceType.TerrainDetail:
                            foreach (DetailPrototype terrainDetail in m_resourceImportTerrain.terrainData.detailPrototypes)
                            {
                                ...
                                protoDetail.m_bendFactor = terrainDetail.bendFactor;
                                newRule.m_imageMasks = new ImageMask[0];
                                //always fold out the resource settings for new rules so user can edit directly
                                newRule.m_isFoldedOut = false;
                                newRule.m_resourceSettingsFoldedOut = false;
        ...
        private void DrawRuleResourceSettings(SpawnRule rule, int spawnRuleID, bool helpEnabled)
        {
            ...
            {
                m_probePrototypeBeingDrawn = m_spawner.m_settings.m_resources.m_probePrototypes[rule.m_resourceIdx];
            ...
            switch (sr.m_resourceType)
            {
                case SpawnerResourceType.TerrainTexture:
                    ...
                    foreach (TerrainLayer proto in activeTerrain.terrainData.terrainLayers)
                    {
                        if (oldTexture != null)
                        {
                            if (PWCommon3.Utils.IsSameTexture(oldTexture, proto.diffuseTexture, false))
                            {
                ...
                case SpawnerResourceType.TerrainDetail:
                    ...
                    if (oldTexture != null || oldGameObject != null)
                    {
                        ...
                        foreach (DetailPrototype proto in activeTerrain.terrainData.detailPrototypes)
                        {
                            ...
                            {
                                if (PWCommon3.Utils.IsSameTexture(oldTexture, proto.prototypeTexture, false) == true)
                                {
                                    detailPrototypeID = localTerrainIdx;
                                    break;
                                }
        ...
        {
#if CTS_PRESENT
                   GaiaResourceEditor.DrawTexturePrototype(m_textureResourcePrototypeBeingDrawn, m_editorUtils, showHelp, m_spawner.ConnectedCTSProfile!=null);
#else
        ...
        private void DrawTreePrototype(bool showHelp)
        {
            ...
            GUILayout.Space(18);
            bool currentGUIState = GUI.enabled;
            if (m_spawner.m_settings.m_spawnerRules[m_spawnRuleIndexBeingDrawn].m_usesBoxStandIn)
            {
            ...
            if (m_spawner.m_settings.m_spawnerRules[m_spawnRuleIndexBeingDrawn].m_usesBoxStandIn)
            {
                ...
                {
                    m_spawner.DeactivateTreeStandIn(m_spawnRuleIndexBeingDrawn);
                }
            }
            else
            {
                ...
                {
                    m_spawner.ActivateTreeStandIn(m_spawnRuleIndexBeingDrawn);
                }
            }
            EditorGUILayout.EndHorizontal();
        }
        ...
        {
            GaiaResourceEditor.DrawProbePrototype(m_probePrototypeBeingDrawn, m_editorUtils, m_gaiaSettings.m_currentRenderer, showHelp);
        }
        ...
        private void DrawSaveAndLoad(bool obj)
        {
            ...
            if (m_editorUtils.Button("LoadButton"))
            {
                ...
                string path = AssetDatabase.GUIDToAssetPath(m_spawner.m_settings.lastGUIDSaved);
            ...
            if (m_editorUtils.Button("SaveButton"))
            {
                ...
                string dialogPath = AssetDatabase.GUIDToAssetPath(m_spawner.m_settings.lastGUIDSaved);
                ...
                if (saveConditionsMet)
                {
                    ...
                    //    AssetDatabase.SaveAssets();
                    //}
                    m_spawner.m_settings.lastGUIDSaved = AssetDatabase.AssetPathToGUID(saveFilePath);
