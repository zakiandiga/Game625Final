...
namespace Gaia
{
    ...
    public class GaiaManagerEditor : EditorWindow, IPWEditor
    {
        ...
        private bool m_useWorldDesigner;
        ...
        {
            Application.OpenURL("https://www.procedural-worlds.com/products/indie-products/gaia-gaia-pro/gaia-extensions/");
        }
        #endregion
        ...
        public void OnEnable()
        {
            ...
            if (m_settings == null)
            {
                m_settings = (GaiaSettings)PWCommon3.AssetUtils.GetAssetScriptableObject("GaiaSettings");
                if (m_settings == null)
                {
            ...
            {
                m_settings.m_currentDefaults = (GaiaDefaults)PWCommon3.AssetUtils.GetAssetScriptableObject("GaiaDefaults");
        ...
        public bool CheckForSetupIssues()
        {
            ...
            //Is the rendering path / color space correct?
            ...
            if (m_foldOutWorldSizeSettings)
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    GUILayout.BeginVertical();
                    {
                        ...
                        {
                            GUILayout.Space(EditorGUIUtility.labelWidth + offSet);
                            m_useWorldDesigner = m_editorUtils.Toggle("UseWorldDesigner", m_useWorldDesigner);
                        }
                        GUILayout.EndHorizontal();
                        GUILayout.BeginHorizontal();
                        {
                            GUILayout.Space(EditorGUIUtility.labelWidth + offSet);
                            m_editorUtils.InlineHelp("UseWorldDesigner", helpEnabled);
                        }
                        GUILayout.EndHorizontal();
#if !GAIA_PRO_PRESENT
                        GUI.enabled = false;
        ...
        private List<Spawner> CreateBiome(BiomePresetDropdownEntry selectedPresetEntry)
        {
            ...
            if (selectedPresetEntry.biomePreset != null)
            {
                ...
                if (GaiaUtils.CheckIfSceneProfileExists())
                {
                    GaiaGlobal.Instance.SceneProfile.m_cullingProfile = selectedPresetEntry.biomePreset.GaiaSceneCullingProfile;
                }
        ...
        void ExportWorldAsHeightmapPNG()
        {
            ...
            if (mgr.TileCount > 0)
            {
                ...
                path = Path.Combine(path, PWCommon3.Utils.FixFileName(string.Format("Terrain-Heightmap-{0:yyyyMMdd-HHmmss}", DateTime.Now)));
        ...
        public static void CreateGaiaExtras(GaiaSettings gaiaSettings, GaiaLightingProfile lightingProfile, List<string> waterList, int selectedWater)
        {
            ...
            if (gaiaSettings.m_currentController != GaiaConstants.EnvironmentControllerType.None)
            {
                ...
                {
                    GaiaSceneManagement.CreatePlayer(gaiaSettings, "Custom", false, gaiaSettings.m_customPlayerObject, gaiaSettings.m_customPlayerCamera);
                }
                else
                {
                    GaiaSceneManagement.CreatePlayer(gaiaSettings);
                }
            }
            if (lightingProfile.m_selectedLightingProfileValuesIndex != -99)
            {
        ...
        private static GameObject CreateScreenShotter(GaiaSettings gaiaSettings)
        {
            ...
            if (shotterObj == null)
            {
                ...
                shotter.m_watermark = PWCommon3.AssetUtils.GetAsset("Made With Gaia Watermark.png", typeof(Texture2D)) as Texture2D;
        ...
        {
            if (m_editorUtils.ClickableText("InstallXRSupportHelp"))
            {
                Application.OpenURL("https://docs.unity3d.com/Packages/com.unity.xr.interaction.toolkit@0.9/manual/index.html");
            }
        ...
        private void DrawRenderSettings(bool helpEnabled)
        {
            ...
            if (m_settings.m_pipelineProfile.m_activePipelineInstalled != GaiaUtils.GetActivePipeline())
            {
            ...
            {
                //If no other warnings, add in a check for linear deferred lighting
                if (m_settings.m_currentEnvironment == GaiaConstants.EnvironmentTarget.Desktop ||
                  m_settings.m_currentEnvironment == GaiaConstants.EnvironmentTarget.PowerfulDesktop ||
                  m_settings.m_currentEnvironment == GaiaConstants.EnvironmentTarget.Custom)
                {
                    ...
                    if (PlayerSettings.colorSpace != ColorSpace.Linear || tier1.renderingPath != RenderingPath.DeferredShading)
                    {
                        EditorGUILayout.HelpBox("This project is not set to Linear Deferred rendering, which is usually NOT recommended unless you are targeting specific devices that do not support linear color space. You can switch to Linear Deferred Rendering with the button below. If you are unsure about this decision, please visit the Link below to read more about Color Spaces in the Unity Manual", MessageType.Warning);
            ...
            if (m_showInstallPipelineHelp)
            {
                GUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                if (m_editorUtils.Button("HideInstallPipelineHelp", GUILayout.Width(220)))
                {
                    m_showInstallPipelineHelp = false;
                }
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            ...
            else
            {
                GUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();
                if (m_editorUtils.Button("ShowInstallPipelineHelp", GUILayout.Width(220)))
                {
                    m_showInstallPipelineHelp = true;
                }
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            }
        ...
        private void DoMaintenance(bool forceMaintenanceRun = false)
        {
            ...
            if ((!System.IO.Directory.Exists(GaiaUtils.GetAssetPath("Dev Utilities")) && File.Exists(GaiaDirectories.GetSettingsDirectory() + "\\" + GaiaConstants.maintenanceTokenFilename)) || forceMaintenanceRun)
            {
                ...
                {
                    if (EditorUtility.DisplayDialog("Install / Update detected", "It looks like you are opening the Gaia Manager the first time after an update or the initial installation. It will now perform a few maintenance tasks to set up Gaia correctly. Do you want to visit a webpage with Gaia tutorials while this process is running?", "Yes, open website", "No thanks"))
                    {
                        Application.OpenURL("https://www.procedural-worlds.com/support/tutorials");
                    }
                }
                //Get Maintenance Profile
                GaiaMaintenanceProfile maintenanceProfile = (GaiaMaintenanceProfile)PWCommon3.AssetUtils.GetAssetScriptableObject("Gaia Maintenance Profile");
